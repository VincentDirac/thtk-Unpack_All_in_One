...
def main():
    # 添加外部参数解析
    parser = argparse.ArgumentParser(description="Process dialogue files with version info.")
    parser.add_argument("--thver", required=True, help="Specify the TH version (e.g., TH17).")
    ...
    for root, folders, files in os.walk(path):
        for file in files:
            for file in files:
                if file in [".gitignore"] or "staff" in file or "vs" in file or "end" in file or "output" in file:
                    continue
                else:
    ...
    with open(os.path.join(root, file_name), "r+", encoding="utf-8") as file:
        ...
        for line in file:
            ...
            if bgm_pattern.search(stripped_line):
                ...
                parsed_lines.append("ja")
                parsed_lines.append(f"{{{{{music_name}音乐名|2|{file_number*2 + 1}}}}}")
                parsed_lines.append("zh")
                parsed_lines.append(f"{{{{{music_name}音乐名|1|{file_number*2 + 1}}}}}")
                continue
